...
async function dissectOne(url, opts = {}) {
  ...
  // ---- 采集（三路并行，带总超时）----
  // 内部超时：3路采集每个最坏情况 40s × 3 retries = 120s，加上 LLM 红蓝对抗 90s，再加 buffer，
  // 整体超时 240s 兜底（防止某个 fetch 永远 hang）
  const body = async () => {
  const [tech, bm, team] = await Promise.allSettled([
    collectTechStack(target, pool),
    collectBusinessModel(target, pool),
    collectTeamSize(target, pool),
  ]);
