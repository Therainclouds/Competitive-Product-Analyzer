 ...
 *   BM_PRECISE           商业模式精修 prompt（v1.6 预备）
...
// ---- 商业模式精修（v1.6 预备，本版先占位） ----

const BM_PRECISE_SYSTEM = `你是一个资深 SaaS 商业模式分析师。
给定一个竞品的定价页文本 + 已识别的变现方式，判断分类是否准确，并补充可能的二级变现模式。`;

function buildBmPreciseUserPrompt(pricingText, identified) {
  return `【定价页文本】
...
请输出 JSON：
{
  ...
  "pricing_tiers_estimate": [{ "tier": "Free/Pro/...", "price": "...", "limit": "..." }],
  "confidence": "high | medium | low",
  "notes": "..."
}`;
}
