 ...
 */
async function probeTheOrg(companySlug) {
  ...
  try {
    ...
    const res = await fetchUrl(url, 20000);
    if (res.status !== 200) {
...
async function probeCrunchbase(companyName) {
...
async function probeSecEdgar(companyName) {
  try {
...
async function collectCompanyInfo(companyName, pool, opts = {}) {
  ...
  const [th, oc, cb, sec, news] = await Promise.allSettled([
    probeTheOrg(slug),
    probeOpenCorporates(companyName),
    probeCrunchbase(companyName),
    probeSecEdgar(companyName),
    probeNewsSignals(companyName),
  ]);
