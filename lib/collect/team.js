...
async function probeGithubOrg(orgNames, deadline = 0) {
  for (const org of orgNames.slice(0, 2)) { // 最多试 2 个候选（慢网络预算）
    try {
      ...
      if (e.budgetExhausted) return { org: null, found: false, budgetExhausted: true };
      /* org 不存在或限流，继续下一个候选 */
    }
...
async function probeCareers(baseUrl, deadline = 0) {
  for (const p of CAREERS_PATHS.slice(0, 3)) { // 最多试 3 个路径（慢网络预算）
    try {
      const url = new URL(p, baseUrl).toString();
      const res = await fetchUrl(url, 15000, 2 * 1024 * 1024, 1, deadline);
      ...
      if (e.budgetExhausted) return { url: null, jobCount: 0, budgetExhausted: true };
      /* 该路径不存在，继续 */
    }
    await new Promise((r) => setTimeout(r, 600));
  }
...
/** 团队规模区间投票（SPECS 第六节 + v1.1 公司信息补强 + v1.2 theorg + v1.8 法定上限裁剪） */
function estimateTeamRange(github, careers, companyInfo) {
  const ranges = [];
  const signals = [];
  const upperBounds = []; // 法定/硬性规模上限（如 UK small company ≤50），裁剪最终区间 max
  ...
  // 0.5. SEC EDGAR（上市公司：10-K 员工数是硬数据；无员工数时 CIK 命中仅作规模下限佐证）
  if (companyInfo && companyInfo.sec_edgar && companyInfo.sec_edgar.found) {
    if (companyInfo.sec_edgar.employees) {
      const n = companyInfo.sec_edgar.employees;
      const r = [Math.floor(n * 0.9), Math.ceil(n * 1.3)]; // 年报快照 ± 披露口径波动
      r.isHard = true; // 硬数据：与弱推断冲突时直接主导区间，不做并集污染
      ranges.push(r);
      signals.push({
        source: 'sec_edgar',
        raw: `10-K 员工数 ${n.toLocaleString()}（filing ${companyInfo.sec_edgar.filing_date || 'N/A'}，硬数据，区间主导）`,
        estimate_range: [r[0], r[1]],
        weight: 0.9,
      });
    } else {
  ...
  // 0.55. UK Companies House 法定规模约束（micro ≤10 / small ≤50 —— 上限裁剪是法律事实）
  if (companyInfo && companyInfo.companies_house && companyInfo.companies_house.found) {
    const ch = companyInfo.companies_house;
    signals.push({
      source: 'companies_house',
      raw: `UK 注册：成立=${ch.incorporation_date} 状态=${ch.company_status}${ch.size_constraint ? `；${ch.size_constraint.label}（法定员工上限 ${ch.size_constraint.max_employees}）` : ''}`,
      estimate_range: ch.size_constraint ? [1, ch.size_constraint.max_employees] : null,
      weight: ch.size_constraint ? 0.8 : 0.4,
      caveat: ch.size_constraint ? undefined : 'registration-only',
    });
    if (ch.size_constraint) upperBounds.push(ch.size_constraint.max_employees);
  }

  // 0.8. OpenCorporates 已弃用（免费 API 申请被拒，2026-09）→ 由 Companies House / SEC 接替其角色
  ...
  // 硬数据存在（SEC 10-K 员工数）→ 直接主导区间，弱推断（招聘页系数/theorg）只作旁证列出
  const hardRanges = ranges.filter((r) => r.isHard);
  if (hardRanges.length > 0) {
    const min = Math.min(...hardRanges.map((r) => r[0]));
    const max = Math.max(...hardRanges.map((r) => r[1]));
    return { range: [min, max], signals, data_sufficient: true, hard_data: true };
  }

  // 多源区间取并集（min of mins, max of maxes）；有法定上限证据时裁剪 max
  const min = Math.min(...ranges.map((r) => r[0]));
  const maxRaw = Math.max(...ranges.map((r) => r[1]));
  const cap = upperBounds.length ? Math.min(...upperBounds) : Infinity;
  const max = Math.min(maxRaw, cap);
  return { range: [Math.min(min, max), max], signals, data_sufficient: true };
}
