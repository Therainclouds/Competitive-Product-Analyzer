...
async function collectTeamSize(url, pool, opts = {}) {
  ...
  const companyInfo = await collectCompanyInfo(companyName, pool, { deadline });
  ...
  const github = await probeGithubOrg(orgNames, deadline);
  if (github.budgetExhausted) pool.add({ source: 'http', kind: 'budget_exhausted', detail: `GitHub org 探测预算耗尽`, url: `https://github.com/${orgNames[0]}` });
  if (github.found) {
  ...
  } else if (!github.budgetExhausted) {
    pool.add({ source: 'github', kind: 'org', detail: `未找到 GitHub org（尝试: ${orgNames.join(', ')}）`, url: `https://github.com/${orgNames[0]}` });
  }

  // 2. 招聘页
  const careers = await probeCareers(base, deadline);
  if (careers.budgetExhausted) pool.add({ source: 'http', kind: 'budget_exhausted', detail: `招聘页探测预算耗尽`, url: `${base}/careers` });
  if (careers.url) {
