...
function assembleTeamSize(team, pool) {
  ...
  // v1.8：SEC 10-K 员工数等硬数据主导时升为 medium（仍是单源快照，不给 high）
  const confidence = team.hard_data ? 'medium' : 'low';
  const caveats = [];
  ...
  let summary;
  if (team.range) {
    const hardNote = team.hard_data ? '（SEC 10-K 硬数据主导，弱推断信号仅作旁证）' : '（低置信度）';
    summary = caveats.length > 0
      ? `估算团队规模区间 ${team.range[0]}-${team.range[1]} 人${hardNote} ⚠️ ${caveats.join('；')}`
      : `估算团队规模区间 ${team.range[0]}-${team.range[1]} 人${hardNote}`;
  } else {
